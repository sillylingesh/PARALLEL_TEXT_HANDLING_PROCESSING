...
async function pollProgress() {
  try {
    ...
    if (!data.running) {
      stopPolling();
      if (data.done > 0) {
        toast(`✅ Processed ${data.done} chunks — batch ${data.batch_id}`, 'success');
        refreshResults();
        refreshCharts();
...
document.getElementById('file-input').addEventListener('change', e => {
  ...
  reader.onload = ev => {
    ...
    toast(`Loaded "${file.name}" — ${ev.target.result.length.toLocaleString()} chars`, 'info');
  };
...
async function runSearch() {
  ...
  let url = `/api/results?label=${encodeURIComponent(label)}`;
...
let _charts = {};
const PAL = { pos: '#10d9a0', neg: '#f43f5e', neu: '#f59e0b', acc: '#6366f1' };
...
async function refreshCharts() {
  try {
    ...
    const chartDefaults = {
      ...
      plugins: {
        legend: { labels: { color: '#6268a8', font: { family: 'Oxanium', size: 11 } } },
        tooltip: {
    ...
    buildChart('chart-bar', 'bar', {
      ...
      scales: {
        x: { ticks: { color: '#6268a8' }, grid: { color: '#1f1f3a' } },
        y: { ticks: { color: '#6268a8' }, grid: { color: '#1f1f3a' }, beginAtZero: true }
      }
    });
    ...
    if (scores.length) {
      ...
      buildChart('chart-hist', 'bar', {
        ...
        plugins: { ...chartDefaults.plugins, legend: { display: false } },
        scales: {
          x: { ticks: { color: '#6268a8', font:{size:10} }, grid: { color: '#1f1f3a' } },
          y: { ticks: { color: '#6268a8' }, grid: { color: '#1f1f3a' }, beginAtZero: true }
        }
      });
    }
    ...
    buildChart('chart-gauge', 'bar', {
      ...
      scales: {
        x: { ticks: { color: '#6268a8' }, grid: { color: '#1f1f3a' }, beginAtZero: true },
