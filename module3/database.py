...
def insert_many(rows_data):
    """
    rows_data: list of tuples (batch_id, chunk, score, label, pos, neg, neu, wc, timestamp)
    """
    if not rows_data:
        return
    conn = sqlite3.connect(DB_PATH)
...
def fetch_all(search_keyword="", min_score=None, max_score=None, sentiment_filter="All", limit=None, offset=0):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
